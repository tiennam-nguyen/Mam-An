...
async function upload(page: Page) {
  ...
  await expect(
    page.getByRole('button', { name: 'Phân tích trực tiếp', exact: true }),
  ).toBeEnabled();
}
for (const width of [390, 1366])
  test.describe(`production client ${width}px`, () => {
    test.use({ viewport: { width, height: 900 }, serviceWorkers: 'block' });
    test('live request → correction → portion → persist/reload/history', async ({
      page,
    }) => {
      let calls = 0;
    ...
    test('Vietnamese labels use a consistent font and UI has no engineering residue', async ({
      page,
    }) => {
      await page.goto('/');
      ...
      for (const href of ['/weekly', '/settings']) {
        ...
        expect(
          fonts.filter((font: { glyphCount: number }) => font.glyphCount > 0),
        ).toHaveLength(1);
      }
      await page.screenshot({
        path: `test-results/polished-home-${width}.png`,
        fullPage: true,
      });
      const svg = await (
        await page.request.get('/demo/images/meal.svg')
      ).text();
      expect(svg).not.toContain('<text');
      for (const path of ['/analyze', '/about', '/settings', '/demo']) {
        await page.goto(path);
        await expect(page.locator('body')).not.toContainText(
          /MockLLM|fixture|Build local|không phải kết quả AI trực tiếp/i,
        );
      }
    });
  });
test.describe('degraded live paths', () => {
  ...
  test('late live result cannot replace a manually corrected draft', async ({
    page,
  }) => {
    let release!: () => void;
    ...
    await page.route('**/api/v2/vision/analyze-meal', async (route) => {
      reached();
      await held;
  ...
  for (const [status, code] of [
    ...
    test(`HTTP ${status} is visible and retry remains live`, async ({
      page,
    }) => {
      let calls = 0;
      await page.route('**/api/v2/vision/analyze-meal', (route) =>
        ++calls === 1
          ? route.fulfill({
              status,
              json: { request_id: 'failed', error: { code, retryable: true } },
            })
  ...
  test('offline failure allows manual entry without silently showing sample results', async ({
    page,
    context,
  }) => {
    await upload(page);
  ...
  test('invalid and oversized uploads leave manual entry available', async ({
    page,
  }) => {
    await page.goto('/analyze');
    for (const buffer of [Buffer.from('invalid'), Buffer.alloc(20000001)]) {
      await page
        .getByLabel('Chọn ảnh bữa ăn')
        .setInputFiles({ name: 'invalid.jpg', mimeType: 'image/jpeg', buffer });
      await expect(page.getByRole('alert')).toBeVisible();
    }
    await page.getByRole('button', { name: 'Nhập món thủ công' }).click();
    await expect(
      page.getByRole('button', { name: '＋ Món tự nhập', exact: true }),
    ).toBeVisible();
  });
  test('storage failure preserves editable draft and can retry', async ({
    page,
  }) => {
