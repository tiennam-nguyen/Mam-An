...
it('minimizes trace and notes; deterministic retrieval and template remain local', () => {
  ...
  const pattern = buildPatternEvidence(d, [], [], 'USER');
  pattern.matchedMealIds = ['PRIVATE ID'];
  const bundle = buildEvidenceBundle(d, pattern, data.knowledge),
    p = minimizeEvidence(bundle);
  expect(JSON.stringify(p)).not.toMatch(
    /PRIVATE|matchedMealIds|glucoseObservations|readingId/,
  );
  expect(templateExplanation(p).generationMode).toBe('TEMPLATE');
  expect(retrieveKnowledge(data.knowledge, ['portion', 'starch'])[0]!.id).toBe(
    'portion',
  );
});
it('rejects invented numbers, unsafe prose, false personal certainty and unknown refs', () => {
  ...
  for (const summaryVi of [
    'Có 999 g carb.',
    'Tăng liều insulin.',
    'Món này gây tăng đường huyết.',
    'Phương án an toàn.',
  ...
  expect(
    validateGeneratedExplanation({ ...out, evidenceRefs: ['unknown'] }, p),
  ).toBe(false);
});
