...
const empty: CapabilityPolicies = {
...
it('v2 image multipart returns components without nutrition and echoes a bounded request id', async () => {
  ...
  form.set('image', new Blob([new Uint8Array(bytes)], { type: 'image/jpeg' }));
  form.set('request_id', 'v2-test');
  ...
  const response = await handler(
    new Request('http://localhost/api/v2/vision/analyze-meal', {
      method: 'POST',
      body: form,
    }),
  );
  ...
  expect(body.request_id).toBe('v2-test');
  expect(body.candidates[0].suggested_components[0].role).toBe('STARCH');
  expect(JSON.stringify(body)).not.toMatch(/carb|kcal/);
});
it('capability fallback is bounded and empty capabilities are independently unavailable', async () => {
  ...
  const router = new CapabilityRouter({
    ...
    TEXT_EXPLANATION_VI: {
      ...
      candidates: [0, 1, 2].map((i) => ({
        ...
        run: async () => {
          calls++;
          return fail('AI_RATE_LIMITED', 'AI', true);
        },
      })),
    },
  });
  await router.route(
    'TEXT_EXPLANATION_VI',
    minimizeEvidence(buildEvidenceBundle(draft(), null, [])),
...
it('explanation rejects extra private fields and unsafe generated prose', async () => {
      ...
      candidates: [
        {
          ...
          run: async () => {
            calls++;
            return ok({
              summaryVi: 'Dùng 10 đơn vị insulin',
  ...
  const handler = createExplanationHandler(router);
  const request = (evidence: unknown) =>
    new Request('http://localhost/api/v2/explanations/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ request_id: 'test', evidence }),
    });
  expect(
    (await handler(request({ ...payload, rawHistory: ['private'] }))).status,
  ).toBe(400);
  expect(calls).toBe(0);
  const response = await handler(request(payload));
  expect(response.status).toBe(502);
  expect(JSON.stringify(await response.json())).not.toContain('insulin');
});
