...
import { formatNumber, ErrorNotice, SafetyNote, Completeness, DemoBadge } from '../../shared/ui/common';
...
export function ReviewPage() {
  ...
  if (!draft || !['REVIEW_REQUIRED', 'REVIEW_READY', 'SAVE_ERROR', 'SAVING'].includes(draft.analysisState)) return <Navigate to="/meal/new" replace />;
  ...
  return <><p className="eyebrow">XEM LẠI & ĐIỀU CHỈNH</p><h1>Bữa ăn theo cách của bạn</h1>{draft.source === 'DEMO_SAMPLE' && <DemoBadge />}<p>Kiểm tra từng thành phần và khẩu phần trước khi lưu.</p>
    ...
    </div><MealEvidence draft={draft} mode={draft.source === 'DEMO_SAMPLE' ? 'DEMO' : 'USER'} /></aside></div></>;
}
