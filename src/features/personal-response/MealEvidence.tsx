...
export function MealEvidence({ draft, mode, excludeId, scenario=null }: { draft:Pick<MealDraft,'entries'|'totalCarbEstimate'|'totalKcalEstimate'|'completeness'>;mode:'USER'|'DEMO';excludeId?:string;scenario?:MealScenario|null }) {
  const { meals,glucose,knowledge,explanations,textExplanationEnabled }=useServices();
  const [generated,setGenerated]=useState<{key:string;value:ExplanationResult}|null>(null),[loading,setLoading]=useState(false);
  ...
  const bundle=buildEvidenceBundle(draft,state.data??null,knowledge,scenario), payload=minimizeEvidence(bundle), fallback=templateExplanation(payload);
  const payloadKey=JSON.stringify(payload);
  useEffect(()=>{ controller.current?.abort(); setLoading(false); return ()=>controller.current?.abort(); },[payloadKey]);
  const explanation=generated?.key===payloadKey?generated.value:fallback;
  return <><ErrorNotice error={state.error} />{state.loading&&<p role="status">Đang xem dữ liệu trên thiết bị…</p>}{state.data&&<PatternCard pattern={state.data} />}<section className="card"><h2>Giải thích từ dữ liệu</h2><small>{explanation.generationMode==='TEMPLATE'?'Mẫu trên thiết bị':'Diễn đạt tự động từ dữ liệu'}</small>{textExplanationEnabled&&<><p>Chỉ khi bạn chọn: gửi phần tóm tắt và số liệu tổng hợp để diễn đạt. Không gửi ghi chú hoặc toàn bộ nhật ký.</p><button disabled={loading} onClick={async()=>{controller.current?.abort();const active=new AbortController();controller.current=active;setLoading(true);const value=await explanations.generateExplanation(payload,active.signal);if(!active.signal.aborted){setGenerated({key:payloadKey,value});setLoading(false);}}}>{loading?'Đang diễn đạt…':'Diễn đạt lại bằng AI'}</button></>}<p>{explanation.summaryVi}</p>{explanation.optionExplanationsVi.map((s,i)=><p key={i}>{s}</p>)}{bundle.retrievedKnowledge.map(k=><details key={k.id}><summary>{k.title}</summary><p>{k.body}</p><small>{k.sourceLabel} · {k.knowledgeVersion}</small></details>)}<p className="muted">{explanation.uncertaintyNoteVi}</p></section></>;
}
