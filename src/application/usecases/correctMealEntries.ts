...
export function correctMealEntries(
  draft: MealDraft,
  ...
  if (
    ...
    return draft;
  const calculated = cloneEntries(entries).map((e) => ({
    ...e,
    components: e.components.map((c) =>
      calculateComponent(c, c.foodId ? catalog.getFoodById(c.foodId) : null),
    ),
  }));
  return {
    ...draft,
    entries: calculated,
    items: flattenEntries(calculated),
    ...entryTotals(calculated),
    analysisState: calculated.some((e) =>
      e.components.some(
        (c) =>
          !c.userCorrected &&
