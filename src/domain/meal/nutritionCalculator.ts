...
export function calculateItemNutrition(food: FoodItem | null, portion: number) {
  ...
  return {
    carbEstimate: scale(food?.carbPerServing),
    kcalEstimate: scale(food?.kcalPerServing),
    nutritionState: (food?.carbPerServing == null
      ? 'UNKNOWN'
      : 'KNOWN') as NutritionState,
  };
}
export function calculateMealNutrition(items: readonly MealDraftItem[]) {
  ...
  const sum = (field: 'carbEstimate' | 'kcalEstimate') => {
    const values = included.flatMap((item) =>
  ...
  return {
    totalCarbEstimate: sum('carbEstimate'),
    totalKcalEstimate: sum('kcalEstimate'),
    completeness: !known
      ? ('UNKNOWN' as const)
      : known === included.length
        ? ('COMPLETE' as const)
        : ('PARTIAL' as const),
  };
}
