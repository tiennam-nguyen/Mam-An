...
export interface MealScenario {
  ...
  resultingEntries: readonly MealEntry[];
  before: ReturnType<typeof entryTotals>;
  after: ReturnType<typeof entryTotals>;
  carbDeltaVsBaseline: number | null;
  kcalDeltaVsBaseline: number | null;
}
...
export function createScenario(
  ...
  operations.forEach((op, index) => {
    if (op.type === 'ADD_COMPONENT') {
      const entry = entries.find((e) => e.entryId === op.targetEntryId),
        food = catalog.getFoodById(op.foodId);
      if (!entry || !food) throw new Error('Missing add target');
      validateUnit(food.id, op.portion, catalog);
      const componentId = id + ':added:' + index;
      if (
        entries.some((e) =>
          e.components.some((c) => c.componentId === componentId),
        )
      )
      ...
      labels.push('Thêm ' + food.nameVi);
      return;
    }
    const entry = entries.find((e) =>
      e.components.some((c) => c.componentId === op.targetComponentId),
    );
    const current = entry?.components.find(
      (c) => c.componentId === op.targetComponentId,
    );
    if (!entry || !current) throw new Error('Missing component');
    if (op.type === 'REMOVE_COMPONENT') {
      entry.components = entry.components.filter(
        (c) => c.componentId !== current.componentId,
      );
      labels.push('Bỏ ' + current.displayName);
      return;
    }
    const foodId =
      op.type === 'REPLACE_COMPONENT' ? op.replacementFoodId : current.foodId;
    const food = foodId ? catalog.getFoodById(foodId) : null;
    if (foodId && !food) throw new Error('Missing food');
    validateUnit(foodId, op.newPortion, catalog);
    ...
    labels.push(
      op.type === 'REPLACE_COMPONENT'
        ? 'Thay ' + current.displayName + ' → ' + food!.nameVi
        : current.displayName +
            ': ' +
            op.newPortion.quantity +
            ' × ' +
            op.newPortion.displayLabelSnapshot,
    );
  });
  const before = entryTotals(baseline.entries),
    after = entryTotals(entries);
  const delta = (a: number | null, b: number | null) =>
    a === null || b === null ? null : b - a;
  return {
    id,
    baseSessionId: baseline.sessionId,
    baselineKey: key(baseline),
    baseline: {
      ...baseline,
      entries: cloneEntries(baseline.entries),
      items: baseline.items.map((i) => ({ ...i })),
    },
    operations: operations.map((o) => ({
      ...o,
      ...('newPortion' in o ? { newPortion: { ...o.newPortion } } : {}),
      ...('portion' in o ? { portion: { ...o.portion } } : {}),
    })),
    operationLabels: labels,
    resultingEntries: entries,
    before,
    after,
    carbDeltaVsBaseline: delta(
      before.totalCarbEstimate,
      after.totalCarbEstimate,
    ),
    kcalDeltaVsBaseline: delta(
      before.totalKcalEstimate,
      after.totalKcalEstimate,
    ),
  };
}
export function applyScenario(
  ...
  if (
    scenario.baseSessionId !== baseline.sessionId ||
    scenario.baselineKey !== key(baseline) ||
    !['REVIEW_READY', 'SAVE_ERROR'].includes(baseline.analysisState)
  )
...
export function discardScenario(baseline: MealDraft): MealDraft {
  return {
    ...baseline,
    entries: cloneEntries(baseline.entries),
    items: baseline.items.map((i) => ({ ...i })),
