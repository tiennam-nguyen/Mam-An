...
export class HttpAiGateway implements AiGateway {
  ...
  async generateExplanation(
    ...
    const fallback = templateExplanation(payload),
      deadline = new AbortController(),
      timer = setTimeout(() => deadline.abort(), this.timeoutMs),
      combined = combineSignals(signal, deadline.signal);
    try {
      const response = await this.send.call(
        globalThis,
        '/api/v2/explanations/generate',
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ request_id: newId(), evidence: payload }),
      ...
      const raw = await response.json(),
        parsed = ExplanationOutputSchema.safeParse(raw.explanation);
      return parsed.success &&
        validateGeneratedExplanation(parsed.data, payload)
        ? { ...parsed.data, generationMode: 'LLM' }
        : fallback;
    } catch {
      return fallback;
    } finally {
  ...
  understandMealImage(
    ...
    return this.analyzeMealImage(input, signal, 2);
  }
  async analyzeMealImage(
    ...
    if (version === 1) form.set('locale', input.locale);
    else form.set('request_id', newId());
    ...
    try {
      const response = await this.send.call(
        globalThis,
        version === 1 ? '/api/v1/analyze-meal' : '/api/v2/vision/analyze-meal',
        {
          method: 'POST',
          body: form,
          signal: combined.signal,
