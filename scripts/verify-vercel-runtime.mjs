...
const entrypoint = process.argv[3] ?? 'api/v1/analyze-meal.ts';
if (
  ![
    'api/v1/analyze-meal.ts',
    'api/v2/vision/analyze-meal.ts',
    'api/v2/explanations/generate.ts',
  ].includes(entrypoint)
)
  throw new Error('Unsupported verification entrypoint');
...
const paths = execFileSync(
  ...
  .filter(
    (path) =>
      /^(api|server|src)\//.test(path) ||
      ['package.json', 'package-lock.json', 'tsconfig.json'].includes(path),
  );
const files = Object.fromEntries(
  paths.map((path) => [path, new FileFsRef({ fsPath: resolve(path) })]),
);
...
console.log(
  JSON.stringify({
    started,
    entrypoint,
    builder: require(resolve(runtime, 'package.json')).version,
    handler: result.output.handler,
    runtime: result.output.runtime,
    fileCount: Object.keys(result.output.files).length,
  }),
);
try {
  ...
  const response = await handler.fetch(
    new Request('http://localhost/api/v1/analyze-meal'),
  );
  console.log(
  ...
  if (entrypoint.includes('/v1/')) form.set('locale', 'vi-VN');
  else form.set('request_id', 'emitted-artifact-check');
  if (entrypoint.includes('/explanations/')) {
    const explanation = await handler.fetch(
      new Request('http://localhost/api/v2/explanations/generate', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          request_id: 'emitted-artifact-check',
          evidence: {
            meal: {
    ...
    console.log(
      JSON.stringify({
        boundary: 'emitted explanation Function → empty capability',
        status: explanation.status,
        body: await explanation.json(),
        noStore: explanation.headers.get('cache-control'),
      }),
    );
    if (explanation.status !== 503) process.exitCode = 1;
  } else {
    ...
    console.log(
      JSON.stringify({
        boundary:
          'emitted Function → sharp → real adapter → stub HTTP → normalized response',
        status: valid.status,
        candidateCount: body.candidates?.length,
        noStore: valid.headers.get('cache-control'),
      }),
    );
    if (valid.status !== 200 || body.candidates?.[0]?.raw_name !== 'Cơm trắng')
      process.exitCode = 1;
  }
} catch (error) {
